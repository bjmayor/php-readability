...
$base_url_list = file("cnblog_blog.txt");
...
foreach ($base_url_list as $base_url)
{
    ...
    while ($i<=$max)
    {
        $listpage = str_replace(PHP_EOL,"",$base_url). "default.html?page=$i";
        echo "listpage:$listpage \n";
        $listcontent = file_get_contents($listpage);
        if($i==2)
        {
        if(preg_match('~<div class="pager">\s*共(\d*)页:~s',$listcontent,$matches))
        {
            $max = $matches[1];
        }
        }
        ...
        if(preg_match_all('~<div class="postTitle">\s*<a id="[^"]*" href="([^"]*)">[^<]*</a>\s*</div>.*?<div class="postFoot">\s*posted @ ([0-9:\-\s]*).*?阅读\((\d*)~s',$listcontent,$matches))
        {
            $j=0;
